...
TextureManager::TextureManager(VkDevice device, VkPhysicalDevice physicalDevice, VkCommandPool commandPool, VkQueue graphicsQueue)
...
TextureManager::~TextureManager() {
    ...
    for (auto memory : textureImageMemories) {
...
bool TextureManager::loadTexture(const std::string& path) {
    ...
    if (!pixels) {
        LOG("Failed to load texture image: " + path);
        return false;
    }
...
VkImageView TextureManager::getTextureImageView(int textureId) const {
    if (textureId < 0 || textureId >= textureImageViews.size()) {
        return VK_NULL_HANDLE;
    }
    return textureImageViews[textureId];
}
...
void TextureManager::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height) {
    ...
    region.imageSubresource.mipLevel = 0; // 0 indexed
